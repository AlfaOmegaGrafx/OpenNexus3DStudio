...
| Concept | RMRK EVM | Character Studio |
|--------|----------|-------------------|
...
2. **RMRK-style NFTs** (MultiAsset + Equippable): use **active assets**, **equipment** (which child is in which slot), and **catalogs** to know which token can go in which slot; then map that to Character Studio trait/slot model.
...
- **Mapping layer**: Config (or manifest extension) that maps:
  - **Collection address** (and optional tokenId/assetId) → Character Studio **trait group** or **"badge" slot**.
  - For RMRK: **Catalog + slot** → one trait group or slot in our model.
- **Assembly**: Existing Character Studio pipeline (manifest + trait IDs) driven by:
...
- **Mapping**:
  ...
  - Map RMRK "slot" (catalog + part/slot id) → Character Studio trait group or badge slot.
...
## 6. Mapping: RMRK → Character Studio Concepts

| RMRK | Character Studio |
|------|-------------------|
...
- **Bridge**: `CharacterStudioBridge` for GLB→Character Studio; keep same pipeline, but **trait/slot source** = wallet + mapping config instead of (or in addition to) static JSON.
...
- [README § Character Studio Features](../README.md#character-studio-features) — Avatar Structure (soulbound base body, layer 0), Wallet-Driven Assembly (planned).
