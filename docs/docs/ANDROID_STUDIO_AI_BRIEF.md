...
**Dev topology:** Vite and `logs/remote-log.txt` run on the **Surface Laptop** (LAN IP e.g. `10.0.0.32`). The **DGX Spark** is reached headless via **NVIDIA Sync** / SSH (`sifr`), not the desktop browser for webcam tests. The **Galaxy XR** headset is typically `10.0.0.224` in logs. See [`DEV_MACHINE_TOPOLOGY.md`](./DEV_MACHINE_TOPOLOGY.md).
