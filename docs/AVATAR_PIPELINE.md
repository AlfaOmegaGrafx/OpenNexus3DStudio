# Avatar pipeline (Character Studio)
...
**Yes — it downloads a file.** The API returns a rigged **GLB**. Character Studio’s existing `VRMExporter` (Save panel or post-pipeline hook) builds a **`.vrm`** blob and triggers a **browser download**. Nothing is uploaded unless you Mint/save elsewhere.
...
- Character Studio skips auto 180° re-orient and rig-repair heuristics for `fromAigc` loads  
